...
<div>
    <table class="table table-sm table-responsive table-hover table-striped" style="font-size:12px" id="suppliers_list_table"
        ...
        data-cookie="true" data-cookie-id-table="SuppliersListTableState" data-cookie-storage="localStorage"
                ...
                <tr data-id="{{ $row['supplier_id'] }}">
                    @foreach ($db_columns as $column_data)
                        @if ($column_data != 'state')
                            <td data-editable="true" class="editable" data-id="{{ $supplier_id }}"
